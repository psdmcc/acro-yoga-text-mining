...
\usepackage{graphicx}           % Core graphics engine for images
\usepackage[dvipsnames,table]{xcolor} % Loads extended color names like teal

% -- UNIFIED CITATION AND LINKING CONFIGURATION (FIXED) -- \usepackage[round,authoryear]{natbib} % Safe, formatted author-year engine
\usepackage{url}                      % Handles special characters in URLs gracefully
\usepackage[colorlinks=true,
            linkcolor=blue,
            citecolor=blue,
            urlcolor=blue]{hyperref}   % Clickable, unified hyperlink routing


 
% -- Safe Font Fallback Configuration -- \setmainfont{Latin Modern Roman}[
     Extension = .otf,
...
% -- Safe Keywords Command Definition -- \providecommand{\keywords}[1]{\textbf{\textit{Keywords -- }} #1}

% -- Code Listing Aesthetics Setup -- \lstset{
...
This structural sanitization process finds absolute text-critical and philological backing when read against the macro-historical overview of premodern somatic systems \cite{Mallinson2017}. Dismantling the popular, ahistorical consensus that defaults to the \textit{Pātañjalayogaśāstra} as the organic practical foundation of physical yoga, comprehensive textual tracking reveals that Patañjali's system was in fact a partisan, elite Brahmanical appropriation of extra-Vedic, \textit{Śramaṇa} Buddhist meditational techniques focused strictly on philosophy rather than physical practice \cite{Mallinson2017}. The actual mechanical precursors to dynamic embodiment -- namely non-seated contortions (\textit{āsana}) and visceral energetic locks (\textit{mudrā}) -- emerged out of the aggressive, difficult physical milieus of peripatetic ascetics whose techniques carried literal connotations of force, trauma, and bodily preservation (\textit{haṭha}/\textit{tapas}) \cite{Mallinson2017}. Diachronic timeline mapping confirms that these volatile, rogue physical capabilities were compiled across formative eleventh-century works like the \textit{Amṛtasiddhi} centuries before they were forcefully flattened and alphabetized into the late-medieval scholastic recensions of the \textit{Siddhasiddhāntapaddhati} and derivative \textit{Yoga Upaniṣads} \cite{Birch2024, Mallinson2017}. The Contortionist Turn thus unmasks a calculated civilizational erasure: the dangerous survival tactics engineered by criminalized frontier outgroups to withstand physical trauma were systematically captured, enclosed, and moralized by elite text-composers to serve as the sanitized, static display technologies of contemporary wellness capitalism \cite{Debnath2024, Mallinson2017, McCartney2021}.
...
\subsection{The Vedic Bedrock: Seasonal Martial Mobilization, Frontier Raids, and Post-Monsoon Harvesting and Seasonal Warfare}
...
The deep structural alignment between the seasonal kinetics of \textit{yoga-k\textsubscript{s}ema} and environmental cycles is explicitly grounded by the macro-historical chronology of the Puranic Autumn Goddess Festival. As Einoo establishes across his comparative survey of twenty-six text layers, the performance of the festival is lexicographically restricted to the first ten days of the bright fortnight of the autumn month of \={A}\textsubscript{s}vina \cite{Einoo1999}. This specific temporal positioning -- occurring precisely when the monsoons have cleared and the rice harvest initiates -- reflects the exact geopolitical transition model where agrarian state units shifted from defensive local preservation (\textit{k\textsubscript{s}ema}) to aggressive, cross-border resource mobilization (\textit{yoga}) along unstable trade thresholds.

This somatic trajectory towards localized physical subversion finds explicit textual backing in the development of the \textit{\textsubscript{s}atrubali} ritual complex recorded within the older chronological strata of the \textit{Dev\={\i}}, \textit{Garu\textsubscript{d}a}, and \textit{Agni Pur\={a}\textsubscript{n}as}. Einoo isolates a highly aggressive, state-sanctioned mechanical sequence where a king, after executing animal sacrifices while chanting specialized \textit{K\={a}l\={\i}} mantras, constructs a physical dough-effigy of his geopolitical adversaries (\textit{p\textsubscript{\={\i}}\textsubscript{s}\textsubscript{t}amaya\textsubscript{m} \textsubscript{s}atrum}) to systematically pierce it with a sword and offer it up to the war-deities Skanda and Vi\textsubscript{s}\={a}kha \cite{Einoo1999}. Rather than an abstract, disembodied theological performance, this ritualized execution maps directly back to the physical joint-crushing and skeletal compression techniques (\textit{ni\textsubscript{s}pipe\textsubscript{s}a}) derived from unrestricted hand-to-hand combat and the strategic subversion of adversarial field forces.

\subsection{The Yogin\={\i} Axis: Peripatetic Acrobatics, Inversions, and Pole Climbing}
The porous boundary separating unstable, peripatetic subaltern performers from religious ritual specialists is explicitly documented across early medieval Puranic tracking configurations. As preserved within the \textit{K\={a}\'{s}\={\i}-kha\textsubscript{n}\textsubscript{d}a} section of the \textit{Skanda-Pur\={a}\textsubscript{n}a} (\textit{SP 4.1.45.11}), the localized physical capabilities of these marginal female performance networks are explicitly itemized before their central theological sanitization: the text details that ``one of them became a clever woman climbing a bamboo pole and another a rope-walker'' \cite{Shastri1991}. Chitgopekar notes that these figures were historically indexed as active participants in raw street spectacles -- including playing the \textit{m\textsubscript{r}da\.{n}ga} drum -- prior to their deliberate absorption into the Śaiva elite apparatus \cite{Chitgopekar2002}. 

This systematic containment reaches encyclopedic codification during the 13th-century Deccan ritual consolidations under Hem\={a}dri. As Dehejia highlights, Hem\={a}dri’s \textit{Caturvarga Cint\={a}ma\textsubscript{n}i} preserves a rare 64-verse liturgical list designated as the \textit{Ak\textsubscript{s}obhy\={a}-\d{R}k\textsubscript{s}akar\textsubscript{n}\={\i}-\d{R}\={a}k\textsubscript{s}as\={\i}} registry, which details fearsome, non-orthodox entities executing specific structural postures: the archer-configured \textit{K\={a}rmuk\={\i}}, the crow-sighted \textit{K\={a}rad\textsubscript{r}\textsubscript{s}\textsubscript{t}i}, and the inverted, face-down \textit{Adhomukh\={\i}} form \cite{Dehejia1986}. 

Furthermore, the mechanical translation of these acrobatics into esoteric yoga registers is directly anchored by the vocabulary of the early corporate Western Kaula stream. As preserved within pa\textsubscript{t}alas 14–16 of the \textit{Kubjik\={a}matatantra} and diagrammatic layout grids of the \textit{Matottaratantra} (such as \textit{Mss. 28/179} from the Sarasvati Bhavan Library), the capacity to leap and balance is textually re-coded as advanced magical-somatic achievements (\textit{siddhis}) \cite{Dehejia1986}. The texts isolate the technical concepts of moving exclusively within the sky or void (\textit{vyomaika-cara\textsubscript{n}a} / \textit{Vyomaika}, verse 52) immediately alongside vertical structural locking or looking upward (\textit{cara\textsubscript{n}ordhvad\={u}k} / \textit{\={u}rdhva-dhrukku}, verse 53). Rather than disembodied mystical metaphors, these strict terms operate as an archival link to physical conditioning paradigms (\textit{v\={a}yuvega}) that regularized the requirements of high-line lever suspension.

\subsection{Somatic Emblems: V\={a}hanas as Tribal Totems and Coalitional Infrastructure}
The systemic cataloging of zoomorphic mounts (\textit{v\={a}hanas}) assigned to individual figures within these registries reflects an operational mechanism of inter-tribal grouping rather than an abstract theological innovation. As Abbasi and Tiwari demonstrate, these animal vehicles functioned originally as active totems or emblems for specific localized tribes and clans throughout Central India \cite{Abbasi2001}. When different tribal groups sharing identical totemic emblems interacted within localized boundaries, the shared emblem minimized cultural friction, cultivating a deep sense of camaraderie and building a broader, integrated foundation of communal worship \cite{Abbasi2001}. By systematically encoding these regional totems into the formal iconographic manuals of the central courtly networks, elite text-composers did not merely document localized folklore; they constructed a scalable, state-sanctioned coalitional infrastructure that successfully regularized and synthesized disparate subaltern populations into a unified ritual economy.
...
\subsection{Caste Puranas as Languages of Argument and Structural Contradiction}
This friction between inherited ritual status and manual physical practice is clarified by analyzing the text as an active document of social negotiation rather than a neutral historical record \cite{Das1968}. As Das establishes in her structural analysis of the Western Indian manual tradition, caste-specific Puranas do not operate as simple charters of fixed communal belief; instead, they function as complex ``languages of argument'' designed to resolve fundamental structural contradictions within the caste hierarchy \cite{Das1968}. In the case of the Jye\textsubscript{s}\textsubscript{t}h\={\i}mallas, the text provides a logical model to reconcile their claimed Brahminical lineage with an occupation -- professional wrestling and combat culture -- inherently incompatible with orthodox \textit{varna} boundaries \cite{Das1968}. By arguing that their somatic skills were a divine gift from Krishna meant to protect their internal \textit{dharma} during the lawlessness of the Kaliyuga, the text separates essential spiritual duty from non-essential lifestyle choice to preserve ritual legitimacy \cite{Das1968}.
...
\subsection{The Subaltern Lexicon and Malla-Bh\={a}\textsubscript{s}\={a}}
This linguistic containment is structurally verified by Sandesara's tracking of the internal vernacular registers embedded within the text-critical layers. The technical catalog does not rely on elite grammatical orthopraxy; rather, it introduces a distinct, non-scholastic combat argot explicitly categorized as \textit{malla-bh\={a}\textsubscript{s}\={a}} or \textit{bhalu-bh\={a}\textsubscript{s}\={a}} \cite{Sandesara1948}. This specialized terminology operates as a subaltern linguistic repository for joint-locking and skeletal compression vectors used within wrestling pit gymnasia (\textit{akh\={a}\textsubscript{d}\={a}s}) before it was forced into formal Sanskrit casing by later monastic redactors.

\subsection{The Biomechanical Leverage of Vet: Cane Work and Apparatus Mobility}
The functional mechanics of subaltern frontier survival find concrete architectural validation in early twentieth-century vernacular training manuals that document the intersection of flexible leverage and apparatus athletics. In his 1929 regional monograph \textit{Vet \={A}\textsubscript{n}i Kust\={\i}}, Kh\={a}sg\={\={\i}}v\={a}le codifies the systematic application of cane or reed drills (\textit{vet}) directly alongside high-line pole wrestling maneuvers (\textit{mallakh\={a}\textsubscript{m}b}) \cite{Khasgivale1929}. This structural pairing reinforces our subaltern continuum model, demonstrating that the biological capacity to pivot, spring, and absorb impact using external wooden levers was preserved as an interconnected, functional asset of mobile performer networks (\textit{la\textsubscript{n}ghakas}) long before its total aesthetic normalization by the bourgeois state.

\subsection{The Epistemic Vault: Jaina Bhandars and Non-Orthodox Preservation}
The transmission map of these volatile somatic records relies fundamentally on a highly distinct, cross-sectarian historical buffer mechanism. As Bühler’s text-critical diagnostics demonstrate during his initial manuscript recovery at the Jaisalmer Bhandar, the long-term structural survival of non-popular Brahmanical panegyrics (\textit{caritas}) and technical treatises was explicitly dependent on the catholic collection policies of medieval Jaina monastic communities \cite{Buhler1875}. Hidden within subterranean temple vaults to escape frontier military raids, these institutional enclosures acted as a supreme historical bottleneck, accidentally protecting the secular, courtly, and material records of statecraft from localized orthoprax erasure.


\subsection{The Kautilyan Matrix: Clandestine Extraction and \textit{Jambhakavidyā}}

By the Maurya era, this volatile somatic substrate of the frontier was systematically secularized, institutionalized, and integrated into the centralized espionage operations of the early state \cite{ArthaŚ}. In a critical diagnostic hit from Kautilya’s \textit{Arthaśāstra} (1.12.7), the sovereign’s premier undercover stationary agents (\textit{sattriṇaḥ}) are commanded to master a highly specialized technical curriculum to maintain their performative operational cover along international transit trade lanes \cite{ArthaŚ}:
\begin{quote}
    \textit{ye cāpy asaṃbandhino ’vaśyabhartavyāste lakṣaṇam aṅgavidyām} \\
    \textit{jambhakavidyāṃ māyāgatam āśramadharmaṃ nimittam... (Arthaśāstra 1.12.7)}
\end{quote}
...
The earliest high-density intersection isolated on our spatial scatter plot drops squarely into Kautilya’s \textit{Arthaśāstra} (2.5.7). Long before the physical pillar (\textit{stambha}) was celebrated as an asset for martial conditioning or internal yoga alignment, it operated as a literal instrument of state incarceration and panoptic surveillance \cite{ArthaŚ}. The text explicitly mandates the physical parameters required to construct the centralized imperial treasury and state prison complex \cite{ArthaŚ}:
\begin{quote}
    \textit{pakveṣṭakāstambhaṃ catuḥśālam ekadvāram anekasthānatalaṃ vivṛtastambhāpasāram ubhayataḥ paṇyagṛhaṃ koṣṭhāgāraṃ ca dīrghbahuśālaṃ kakṣyāvṛtakuḍyam antaḥ kupyagṛham tad eva bhūmigṛhayuktam āyudhāgāraṃ pṛthagdharmasthīyaṃ mahāmātrīyaṃ} \\
    \textit{vibhaktastrīpuruṣasthānam apasārataḥ suguptakakṣyaṃ bandhanāgāraṃ kārayet (Arthaśāstra 2.5.7)}
\end{quote}
...
First, a blistering fast regular expression framework strips out raw semantic textual data from localized layout markup boilerplate:
\begin{equation}
...
\begin{equation}
\mathcal{R}_{\text{clean}} = \text{\texttt{[\^{}\textbackslash w\textbackslash s\textbackslash u0300-\textbackslash u036f\textbackslash u1e00-\textbackslash u1eff]}}
\end{equation}
...
Rather than calculating standard, document-wide Jaccard metrics -- which are inherently skewed by document-length variance and introduce significant noise over massive textual files -- this pipeline introduces a localized \textit{sliding-window Jaccard coefficient}. When any target keyword $T_i$ matches a token at index $k$ within the continuous stream, a strict symmetrical proximity boundary window size parameter ($N = \pm 5$ words) is established around the coordinate. 
...
The structural configurations of the multi-corpus network are empirically exposed through two complementary visualization nodes: a static sliding-window collocation matrix (\textit{somatic\_overlap\_matrix.png}) and an interactive force-directed graph physics dashboard (\url{https://psdmcc.github.io/acro-yoga-text-mining/somatic_network.html}), both hosted live within the open-source repository infrastructure \cite{McCartney2021}. Rather than defaulting to an ahistorical, disembodied model of spiritual evolution, the topological centrality curves document a calculated top-down technological extraction pipeline \cite{Mallinson2017}. The matrix demonstrates a severe, non-linear co-occurrence density profile where specialized somatic keywords overlap with explicit variables of clandestinity, toxicology, and subaltern performance along the expansionist frontier of the early state \cite{Mallinson2017, McCartney2021}. 

Concurrently, the force-directed physics layout models the precise structural distances and transmission vectors running across these distinct socio-historical clusters \cite{Lorenzen2011}. As visualized in the graph topology, the independent alchemical internalizations of the eastern delta (\textit{cāri-candra} $\rightarrow$ \textit{bindu}) and the manual venom-freezing maneuvers of marginalized snake-handlers (\textit{sapera} $\rightarrow$ \textit{viṣa-stambhana}) converge directly upon the central Kautilyan espionage hubs \cite{Lorenzen2011, Mallinson2017}. The state apparatus systematically vampirized these rogue survival assets, re-coding subaltern contortion (\textit{ḍomba}) and lethal anatomy (\textit{aṅgavidyā}) into an elite technical curriculum for counter-surveillance field agents (\textit{gūḍhapuruṣa}) managed within architectural containment grids (\textit{bandhanāgāra}) \cite{McCartney2026b}. Crucially, the graph unmasks a supreme transcultural bottleneck corridor where the statecraft metric of \textit{jambhakavidyā} links directly to Hellenistic fluid payloads (\textit{ios}), proving that the institutionalization, sanitization, and moralization of volatile subaltern biology is a universal macro-historical feature of centralizing orthodoxies, whether in the Mediterranean or South Asia \cite{Debnath2024, Lorenzen2011, McCartney2021}.

The interactive force-directed graph physics dashboard detailing these topological cluster distances is fully replicated and hosted live online at \url{https://github.com/psdmcc/acro-yoga-text-mining}.
...
The formal stylistic grafting and strategic ambiguity of these text-critical layers reflect the specific requirements of the pre-British regional courts. Das demonstrates that the internal composition of the \textit{Mallapur\={a}\textsubscript{n}a} mimics the formal logic of a judicial decision-making process because these origin-myths were explicitly designed to serve as empirical evidence before kings and zamindars, who acted as the supreme legal arbiters of caste rank, privilege, and boundaries \cite{Das1968}. To legitimize these claims within traditional Sanskritic learning, elite text-composers systematically copied ancient Vedic structures, attached their lineages directly to major texts like the \textit{Skanda Pur\={a}\textsubscript{n}a}, and re-coded localized clan deities -- such as the tree-goddess Nimba-j\={a} -- into the cosmic, orthodox \textit{yogam\={a}y\={a}} of mainstream theological systems \cite{Das1968}.
...
\begin{lstlisting}[caption={Kauśika Paddhati Overlap Configuration}, label={list:kaushika}]
...
\subsection{Scholastic Neutralization: K\textsubscript{s}emar\={a}ja and the Abstraction of Bodily Flight}
The absolute philosophical domestication of these peripatetic mobility assets occurs when raw physical movement is re-coded as internal mental statecraft. Mallinson tracks this corporate lineage through the \textit{Kubjik\={a}matatantra}, which maps a strict systemic hierarchy of feminine kinetic agents: \textit{Dev\={\i}s}, \textit{D\={u}t\={\i}s}, \textit{M\={a}t\textsubscript{r}s}, \textit{Yogin\={\i}s}, and \textit{Khecar\={\in}s} \cite{Mallinson2007}. While the lower tiers remain bound to terrestrial fields -- such as the earth-bound \textit{Bh\={u}car\={\i}} and sensory-bound \textit{Gocar\={\i}} forms codified in the \textit{Kaulaj\~{n}\={a}nanir\textsubscript{n}aya} -- the \textit{Khecar\={\i}} represents the supreme ``sky-dweller'' or sovereign mother of all \textit{siddhiyogin\={\i}s} \cite{Mallinson2007}. 

In his commentary layers -- specifically the \textit{Spandandir\textsubscript{n}aya} (1.20) and \textit{\'{S}ivas\={u}travimar\'{s}in\={\i}} (2.5) -- K\textsubscript{s}emar\={a}ja regularizes these volatile, spatial circles of deities (\textit{devat\={a}cakr\={a}\textsubscript{n}i}) into clean psychological expressions, branding the supreme sky-walker (\textit{Khecar\={\i}}) as the literal embodiment of the highest consciousness (\textit{parasa\textsubscript{m}vitsvar\={u}p\={a}}) \cite{Mallinson2007}. This scholastic capture effectively neutralizes the literal, physical subversion of the leaping acrobat, locking her kinetic agency inside the passive metaphysical boundaries of Monistic Kaśmīr Śaivism.

\subsection{The Rasa\'s\={a}l\={a} as an Architectural Framework for Wealth and Enclosure}
The operational demands of these physical processes required specialized spatial configuration long before modern industrial or administrative institutionalization. Sauthoff’s analysis of Chapter 3 of Somadeva’s \textit{Rasendrac\={u}\textsubscript{d}\={a}ma\textsubscript{n}i} highlights that the alchemical workplace (\textit{rasa\'s\={a}l\={a}}) was highly structured, integrating complex technical apparatuses with altars and managed herb gardens \cite{Wujastyk2025}. Far from being a transient, peripatetic craft, the physical labor of grinding, melting, and distilling mercury required an established, settled geographic presence and substantial material capital. This spatial enclosure demonstrates that advanced physical technologies were historically dependent on sovereign patronage grids and the concentrated accumulation of resource streams, mirroring the precise institutional boundaries that monitored and normalized rogue physical specialists.
...
\subsection{Legal Status and the K\={a}\textsubscript{n}\textsubscript{d}aspr\textsubscript{u}\textsubscript{s}\textsubscript{t}a Segregation}
The structural marginalization of these martial specialists is deeply etched into medieval socio-legal commentary layers, disproving the myth of an organic Brahminical origin. As Sandesara uncovers via Abhayatilakaga\textsubscript{n}i’s commentary on Hemachandra’s \textit{Dvy\={a}\textsubscript{s}raya Mah\={a}k\={a}vya}, weapon-wielding and arms-bearing Brahmins (\textit{\={a}yudhaj\={\i}v\={\i}}) were lexicographically branded with the degrading labels of \textit{K\={a}\textsubscript{n}\textsubscript{d}aspr\textsubscript{u}\textsubscript{s}\textsubscript{t}a} (weapon-touchers) and \textit{Br\={a}hma\textsubscript{n}aka} \cite{Sandesara1948}. These populations were legally segregated into isolated, peripheral border territories, demonstrating that their somatic mastery was initially monitored as a dangerous, volatile asset of statecraft long before its courtly assimilation into orthodox caste matrices.
...
This toxicological and subaltern baseline achieves further historical and sociological validation when integrated with recent field-based monographs tracking the long-term marginalization of householder lineages \cite{Debnath2024}. As Kunal Debnath demonstrates in his diachronic survey of the eastern delta corridors, the early crystallization of the Natha-pantha operated as a highly volatile, anti-domestic intersection of Shaivite asceticism, Hatha Yoga, and the traditional Indian school of alchemy (\textit{rasāyana}) \cite{Debnath2024}. Rather than pursuing an abstract, otherworldly escape from material reality, these early practitioners prioritized \textit{kāya-sādhana} (the intense cultivation of the physical body) to capture absolute immunization and cellular immortality \cite{Debnath2024}. This alchemical framework relied entirely on a radical process of reversal (\textit{ulṭā sādhana}) designed to arrest the downward flow of vital generative fluids (\textit{bindu}) alongside the ingestion and transformation of hazardous mercury oxides within the biological container \cite{Debnath2024}. Consequently, the somatic mechanics of immobilization (\textit{stambha}) and fluid control were firmly bound to an empirical, non-Brahminized landscape of substance management long before their late-medieval monastic sanitization and subsequent modern packaging into bourgeois wellness capitalism \cite{Debnath2024, McCartney2021}.


\section{The Courtly Capture: Postural Weaponization and Caste Shifting}

By the 12th century, our diachronic timeline maps a major structural shift. The raw, volatile, border-crossing body skills of the subaltern were systematically captured and institutionalized by elite regional courts. King Someśvara III’s \textit{Mānasollāsa} (\textit{Mallavinoda} section) captures this transition perfectly, documenting the moment elite martial arts absorbed these physical configurations \cite{Mān_3}:
\begin{quote}
    \textit{mūrdhni kūrmāsanaṃ badhdvā bhajedvāpyekapādake /} \\
    \textit{uttānapratimallasya nivārya caraṇadvayam // (Mānasollāsa 4.2.984)}
\end{quote}
...
\subsection{The Vernacular Coding of Akh\={a}\textsubscript{d}\={a} Grips and Thigh-Clamping}
This tactical continuum bridges the structural distance between medieval Sanskrit classifications of close-quarters joint-locking (\textit{a\.{n}gal\={a}gas}) and modern regional athletic scripts. Kh\={a}sg\={\={\i}}v\={a}le maps explicit, localized physical matrices that detail how a practitioner utilizes precise thigh-clamping durability (\textit{ja\.{n}gh\={a}pr\={a}\textsubscript{n}a}) and micro-anatomical shoulder locks to capture total leverage on both mobile canes and vertical column apparatuses \cite{Khasgivale1929}. By detailing these grueling, vernacular conditioning rules (\textit{\textsubscript{s}ramagu\textsubscript{n}a}) inside the space of the Balodyan Press manuals, the archive empirically anchors the exact physical substrate that late-medieval and modern redactors systematically stripped of its combat-gymnasia realities.

\subsection{The Lohabhih\={a}rika and N\={\i}r\={a}jan\={a} Extraction Lineages: The Biomechanical Mechanics of Niyuddha and Pachāṛ}
The exact biomechanical interface linking vertical apparatus training and the purification of centralized military capital is codified across the technical guidelines of the \textit{Bhavi\textsubscript{s}ya Pur\={a}\textsubscript{n}a}. Einoo unearths an ancient, state-controlled military ritual complex designated as the \textit{lohabhih\={a}rika-karman} or \textit{n\={\i}r\={a}jan\={a}}, where a sovereign’s weapons, armor, horses, and frontline elephants are systematically installed within a dedicated pavilion, treated with alchemical unguents, and paraded through architectural surveillance structures to divine the state's fortunes \cite{Einoo1999}. This statecraft mechanism -- which directly mirrors early Vedic definitions of weaponized mobilization and the manual preservation of vital channels (\textit{marman}) -- underscores that advanced physical containment was structurally engineered to reinforce royal fortification defense frameworks long before its scholastic internalization by late-medieval hatha compilers.
...
This micro-anatomical targeting aligns perfectly with Epic accounts of unrestricted arena combat. In the \textit{Harivaṃśa} (75.42), Kṛṣṇa’s final strike against Caṇūra is defined as an un-weaponized execution (\textit{aśastram atighoraṃ}) consisting of raw physical power (\textit{balaprāṇaviniṣpādyaṃ}) \cite{HV_App}:
\begin{quote}
    \textit{prāharan muṣṭinā mūrdhni vakṣasyāhatya jānunā} \\
    \textit{niḥsṛte sāśrurudhire tasya netre sabandhane (Harivaṃśa 75.42)}
\end{quote}
...
This combat-ready substrate achieves definitive textual and biomechanical verification when executing an un-truncated analysis of the transcribed layers of the \textit{Mallapurāṇa} \cite{MP_7}. Far from reflecting an introspective, disembodied theology of self-transcendence, the vernacular technical lexicon (\textit{mallabhāṣā}) systematically indexes specialized bodily alignment as an active weapon of physical dominance and skeletal destruction \cite{MP_7}. In the structural catalogs of Chapter 6, physical positions (\textit{sthāna}) and manual holds (\textit{laga}) are explicitly coupled with visceral arena combat matrices: \textit{ghātapātastathālātaḥ... sthānakaṃ laga Eva ca} -- tracking strikes, throws, and kicks within a fixed token window \cite{MP_7}. 

This tactical grid is formalized across Chapter 8, which codifies the fourteen distinct mechanical body grips known as the \textit{aṅgalāgas}: \textit{athāṇgalāgāḥ kathyante saṃkhyayā ca caturdaśa} \cite{MP_7}. These vectors map precise close-quarters joint-locking and bone-crushing submission maneuvers, spanning aggressive hair-seizing locks (\textit{jūṭake kathyate lāgaḥ}), rear-line spinal compressions (\textit{pṛṣṭhalāgo vaṃśalāgaḥ}), and supportless abdominal clamps (\textit{ādhāro'pi lāgaḥ}) engineered specifically to capture leverage and arrest an opponent's vital breath (\textit{prāṇa}) \cite{MP_7}. Furthermore, the text-critical alignment of seventeen non-seated contortions, terminating in the core-compressing belly posture (\textit{udarāsanamityevaṃ kathitaṃ daśasaptadhā}), is explicitly contextualized as a rigorous daily physical exercise schedule (\textit{śramaguṇa}) rather than a tool for quiet contemplative union \cite{MP_7}. Coupled with the technical data in Chapter 10 detailing vertical apparatus training rules (\textit{stambhādhirohaṇaṃ}) to cultivate localized shoulder power (\textit{skandhaprāṇa}) and thigh-clamping durability (\textit{jaṅghāprāṇa}), the \textit{Mallapurāṇa} transcriptions document the exact historical substrate of subaltern and martial biology that late-medieval monastic redactors later captured, sanitized, and isolated from its raw material realities \cite{Birch2024, MP_7, Mallinson2017}.

\subsection{The Vindhyav\={a}sin\={\i} Synthesis: Frontier Goddess Assimilation}
This cross-sectarian process of structural capture maps directly onto the early geopolitical distribution of the warrior goddess complex along the Vindhya mountain corridors. As Yokochi establishes through text-critical and iconographic tracking, the early historical trajectory of the non-orthodox goddess who abides in the Vindhya mountains (\textit{Vindhyav\={a}sin\={\i}}) underwent a deliberate fusion with the pan-Indic \textit{Mah\textsubscript{i}\textsubscript{s}\={a}suramardin\={\i}} mythos between the fourth and seventh centuries CE \cite{Yokochi1999}. Both early Vai\textsubscript{s}\textsubscript{n}ava and \'Saiva state apparatuses vied to absorb these fierce, marginal warrior motifs into their respective mythic cycles -- culminating in the original \textit{Skandapur\={a}\textsubscript{n}a} subordinating the localized tree-goddess under P\={a}rvat\={\i}’s domestic mandala \cite{Yokochi1999}. This macro-historical enclosure mirrors the exact trajectory of scholastic capture, where the untamed, defensive violence of the frontier body is progressively harnessed to legitimate royal sovereignty and territorial fortification defense arrays.

The diachronic textual profile registers a massive vocabulary baseline across the parsed text streams, isolating $86,995$ occurrences of the base lemma \textit{yoga}, $5,538$ of \textit{malla}, $5,804$ of \textit{stambha}, and $1,872$ of the alchemical descriptor \textit{m\={a}ra\textsubscript{n}a}. Rolling Jaccard coefficient scoring within a strict sliding proximity boundary (\(\pm 5\) words) mathematically exposes the structural proximity between the martial subject and localized containment configurations, with the \textit{malla-stambha} intersection ($0.0312$) and the \textit{stambha-m\={a}ra\textsubscript{n}a} axis ($0.0367$) far outpacing the abstracted \textit{yoga-m\={a}ra\textsubscript{n}a} baseline ($0.0099$). This divergence quantitatively confirms that physical culture and mercurial laboratory science shared a highly specialized, local subaltern argot long before its tactical containment and subsequent philosophical sterilization by elite scholastic redactors.


\subsection{Intertextual Synthesis and the Technical Orthodoxy of Doing}
To strip these physical skills of their unstable, localized variations, early text-composers built a unified technical orthodoxy that superseded localized religious affiliations. D. Wujastyk tracks this synthesis by noting how later compilations systematically paraphased and borrowed recipes from both the non-sectarian \textit{Rasah\textsubscript{r}dayatantra} and the heavily Śaiva-Kaula \textit{Ras\={a}r\textsubscript{n}ava} \cite{Wujastyk2025}. By isolating the practical workflows of cleansing (\textit{\'sodhana}), essence extraction (\textit{sattvap\={a}tana}), and calcination (\textit{m\={a}ra\textsubscript{n}a}) from their initial ritual and narrative frameworks, these manuals established a standardized canon of physical manipulation. This intertextual distillation flattened the human biological operator into a passive component of a universal scholarly language, ensuring that the kinetic techniques could be cleanly transmitted across regional boundaries under state-sanctioned parameters.

\subsection{The Acoustic Arena: Yogakṣema and the Alternation Model of Combat}

The linguistic foundation of the Contortionist Turn is definitively anchored by tracking the terms \textit{yoga} and \textit{kṣema} back to their earliest attested layers in the Indo-Aryan corpus. As codified in \textit{Mallapurāṇa} (7.1), the acoustic environments and kinetic shouts performed within the training yard are explicitly systematized for material acquisition and political dominance \cite{MP_7}:
\begin{quote}
    \textit{ākhāḍhasya samīpasthā ye kāryāḥ svarāḥ kila |} \\
    \textit{tatsarvaṃ kathayiṣyāmi yogakṣemārthasiddhaye || (Mallapurāṇa 7.1)}
\end{quote}
...
This transition from scholastic text-composers to standardized national disciplines is empirically mapped by diachronic shifts in the medium of transmission. As Thomas demonstrates in his comparison of the Jye\textsubscript{s}\textsubscript{t}h\={\i}malla manual traditions, the early modern \textit{Mallapur\={a}\textsubscript{n}a} (c. 1674–1675) reconciled subaltern identity using ancestral, Puranic mythology \cite{Thomas2025}. However, early twentieth-century print developments like Sir Gangadhar-Rao Ganesh Patwardhan's 1927 treatise, \textit{The Science of Wrestling}, discarded these narrative frameworks for mass-produced photography and explicit Western pedagogical methods \cite{Thomas2025}. This shift systematically flattened localized caste negotiations into highly managed, reproducible body mechanics optimized for broad public consumption.
...
\subsection{Refining the Twelfth-Century Trans-Regional Jyeṣṭhi-malla  Migration Timeline, the Kalyani Fort Nexus, and the Courtly Panegyric Framework under Vikram\={a}ditya VI }
Crucially, diachronic corpus tracking disconfirms the standard historical consensus that limits this southern flight to the aftermath of Alauddin Khalji's sack of Modhera in 1304 CE. Sandesara asserts a highly coordinated, non-linear transmission timeline by mapping the direct structural parallelisms between the \textit{Malla-vinoda} section of Some\textsubscript{s}vara III’s \textit{M\={a}nasoll\={a}sa} (c. 1129 CE) and the Western Indian manual tradition \cite{Sandesara1948}. This early 12th-century alignment, fostered under the reign of Siddhar\={a}ja Jayasi\textsubscript{m}ha, establishes that the state-sponsored extraction of subaltern somatic assets was an active, strategic feature of inter-state networking centuries before geopolitical crises triggered mass territorial displacements.
...
This structural extraction pipeline achieves explicit geopolitical localization when read against the immediate generational precursor to the Chalukyan courtly martial manual tradition. Writing as the imperial court poet (\textit{Vidy\={a}pati}) under Some\'svara III's father, Vikram\={a}ditya VI (Tribhuvanamalla), Bilha\textsubscript{n}a codifies a highly militarized spatial economy where royal power is explicitly validated through cross-border raids and the construction of aggressive display capitals \cite{Buhler1875}. Bilha\textsubscript{n}a's biographical trajectory -- traveling as an elite mercenary intellect on state elephants through Kar\textsubscript{n}\={a}\textsubscript{t}a-land -- documents the precise institutional matrix that stabilized territorial property (\textit{yoga-k\textsubscript{s}ema}) and regularized the localized physical conditioning networks later formalized inside the Kalyani garrison training yards.

To mask the raw, biomechanical realities of hand-to-hand skeletal combat hold extractions, the centralized state apparatus deployed highly formalized, disembodied creation myths designed to project civilizational authority. In Sarga I of the \textit{Vikram\={a}\textsubscript{n}kadevacarita}, the physical origin of the Chalukya lineage is structurally scrubbed of its earthly combat-gymnasia realities; the primeval warrior is explicitly framed as an organic manifestation emerging directly out of Brahm\={a}'s ritual water vessel (\textit{chuluka}) to neutralize battlefield godlessness on earth \cite{Buhler1875}. This cosmic re-coding perfectly illustrates the initial axis of scholastic capture, where the elite text-composer isolates the kinetic violence of frontier boundary management and cleanses it under a layer of sovereign theological panegyric.
...
The structural degradation of indigenous combat gymnasia was deeply accelerated by colonial shifts in state organization. Thomas traces how early modern wrestling systems originally operated within an active regional ``military labor market,'' offering martial specialists fluid paths to political employment and localized state defense \cite{Thomas2025}. Under British colonial centralization, this functional utility was completely severed, decoupling the physical container from direct combat statecraft and re-orienting it toward anti-Western resistance and internal communal containment grids \cite{Thomas2025}.
...
The ultimate enclosure of the rogue body within respectable state structures found an ideal ideological vehicle in the late nineteenth-century physical culture movement. Thomas notes that late-colonial manuals explicitly co-opted traditional wrestling practices to fuel nationalist rhetoric, transforming the training ground into a staging site for anti-colonial resistance and muscular religious nationalism \cite{Thomas2025}. By re-coding the volatile, marginal physical techniques of the \textit{akh\={a}\textsubscript{d}\={a}} into tools for national regeneration, these print networks finalized the domestication of the kinetic subject, trapping it within the political boundaries of the modern nation-state \cite{Thomas2025}.

\subsection{The 1920s Renaissance: Normalization into Bourgeois Spectacular Display}
The terminal capture axis of the Acro-Yoga Complex is definitively anchored by tracking the mid-twentieth-century transformation of regional \textit{akh\={a}\textsubscript{d}\={a}} culture into nationalist physical display programs. The publishing footprint of the Balodyan Press in 1929 marks the precise historical moment where the fluid, dangerous, and criminalized border-evasion skills of the subaltern underbelly were permanently re-coded into a structured, pedagogical syllabus of national pride and preventative healthcare \cite{Khasgivale1929}. By isolating these complex physical postures from their historical contexts of shadow intelligence gathering and military fortification defense, these early modern catalogs finalized the long-term project of theological sanitization, flattening the human biological container into a harmless, respectably bourgeois display technology (\textit{prek\textsubscript{s}a\textsubscript{n}\={\i}ya k\={a}m}).
...
This long-term trajectory of monastic internalization achieves absolute textual and physiological anchoring when read against the diachronic mapping of early vernacular folklore and hatha treatises \cite{Lorenzen2011}. As Lubomír Ondračka uncovers in his analysis of the medieval Bengali recensions of the Matsyendra-Gorakh narrative, the primitive mechanics of somatic preservation pivoted on the volatile, secret manipulation of the four moons (\textit{cāri candra}) \cite{Lorenzen2011}. Operating as a highly technical, subaltern fluid-extraction framework, these elements represent the direct physical control, inversion, and metabolic re-absorption of vital internal humors and sexual fluids within the biological laboratory of the organism, engineered to bypass anatomical decomposition and check the downward path of systemic decay \cite{Lorenzen2011}. 

This focus on complete biological containment is structurally expanded by the macro-corporeal architecture of the \textit{Siddha-siddhānta-paddhati} \cite{Lorenzen2011}. Rather than reducing the human container (\textit{piṇḍa}) to a simple, miniaturized replica of the cosmic egg (\textit{brahmāṇḍa}), David Gordon White demonstrates that early hatha text-composers enforced a literal, absolute enclosure of the entire physical universe straight inside the anatomical channels of the practitioner \cite{Lorenzen2011}. By drawing the external topographies of regional frontiers, mountains, and rivers inward, the yogi establishes an independent, self-contained sovereign monopoly over vital space to achieve absolute corporate equality with the divine \cite{Lorenzen2011}. The historical project of the Contortionist Turn is thus fully exposed: just as Gorakh’s legendary rescue of Matsyendra from the Kingdom of Women functioned as a conscious, structural allegory to expunge the volatile, non-linear sexual rites of early Kaula lineages, late-medieval and modern redactors systematically flattened these dangerous, peripatetic assets of survival into the static, respectably bourgeois display technologies of the contemporary transnational wellness marketplace \cite{Birch2024, Debnath2024, Lorenzen2011}.

\subsection{The Patidar Wedding Ritual as Terminal Flattening}
The ultimate domestic flattening of this martial continuum finds a stark ethnographic baseline in the modern decay profiles of the North Gujarat frontier. Stripped of imperial court patronage, the functional mechanics of combat-ready posture were permanently reduced to a demeaning, token street performance layout: entering the wedding processions of Leuva Patidar elites to execute a single, symbolic wrestling stance (\textit{chapiye d\={a}vu}) in exchange for meager monetary tips (\textit{jama\textsubscript{n}}) \cite{Sandesara1948}. This terminal degradation, which was explicitly banned by the caste association in 1945 to safeguard their social status, perfectly captures the macro-historical shift from weaponized intelligence assets and statecraft to a highly managed display technology (\textit{prek\textsubscript{s}a\textsubscript{n}\={\i}ya k\={a}m}) optimized for civic consumption.
...
The macro-historical validity of the Contortionist Turn (CT) is empirically established by evaluating the non-linear trajectories of our diachronic network centrality curves (\texttt{somatic\_chronology\_timeline.png}) \cite{McCartney2021}. Rather than validating a linear, organic evolution of spiritualized posture from ancient Vedic roots, the normalized group semantic density profiles expose a severe historical inversion across a three-millennium timeline \cite{Mallinson2017, McCartney2021}. During the classical Mauryan and late antique horizons, the \textit{Subversive Mobility Index} (tracking clandestine caravan transit, shamanic flight, and field intelligence vectors) achieves its historical zenith of approximately 68 points near the first century CE, while the adjacent \textit{Somatic Contortion Index} remains entirely dormant on the baseline \cite{Mallinson2017}. This phase maps a volatile, open spatial economy where advanced bodily manipulation operated as a functional asset of statecraft, frontier evasion, and military fortification science before its subsequent scholastic containment \cite{McCartney2026b}.

The mid-medieval horizon initiates the structural capture axis, marked by a sharp drop in independent subaltern mobility and a simultaneous, near-vertical explosion of the \textit{Somatic Contortion Index} beyond the fifteenth century \cite{Debnath2024, Lorenzen2011}. Reaching an unrestricted apex exceeding 105 points, this modern surge charts the terminal sanitization and institutional sterilization of the biological organism \cite{McCartney2021}. As the rogue, peripatetic assets of traveling acrobat guilds (\textit{ḍomba}) and outcaste root-cutters (\textit{śvapāka}) were forcefully crushed by regional courts and early hatha compilers, their dynamic combat holds and toxicological containment tactics (\textit{viṣa-stambhana}) were turned permanently inward \cite{Birch2024, Mallinson2017}. This structural inversion demonstrates that the mechanical components of modern yoga were systematically extracted from a criminalized frontier underbelly, frozen under a sanitized theological veneer, and ultimately repackaged into the static, respectably bourgeois spectacular display technologies (\textit{prekṣaṇīya kām}) of the contemporary global wellness marketplace \cite{Debnath2024, McCartney2021}.
...
Kh\={a}sg\={\={\i}}v\={a}le, Anant Harihara (1929). \textit{Vet \={A}\textsubscript{n}i Kust\={\i} (Mallakh\={a}\textsubscript{m}b)}. Pune: Balodyan Press.
