...
def sort_bib_by_text_appearance_robust(file_path):
    ...
    # Split body after \begin{document}
    ...
    # Isolate using a flexible split pattern on the bibliography block entry point
    body_split = re.split(r"\\begin\{thebibliography\}", body_parts[1], maxsplit=1, flags=re.IGNORECASE)
    body_text = body_split[0]
    ...
    print("Step 2: Parsing bibliography blocks via flexible regular expressions...")
    
    # Flexible regex matches that ignore spacing variation or parameter values like {100} or {99}
    start_match = re.search(r"\\begin\{thebibliography\}\s*\{[^\\}]*\}", content, re.IGNORECASE)
    end_match = re.search(r"\\end\{thebibliography\}", content, re.IGNORECASE)
    
    if not (start_match and end_match):
        print("Error: Structural boundary marker mismatch. Aborting operation to protect your file content.")
        return
...
if __name__ == '__main__':
    ...
    sort_bib_by_text_appearance_robust(file_path=target)
